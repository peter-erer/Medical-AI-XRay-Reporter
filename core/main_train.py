...
def parse_agrs():
    ...
    # --- 核心修复：添加 load 和 retrieval_index_path 参数 ---
    parser.add_argument('--load', type=str, help='the path to the pre-trained model checkpoint.')
    parser.add_argument('--retrieval_index_path', type=str, default='./retrieval_index.json',
                        help='the path to the retrieval index json file.')

    # SCST (强化学习) 设置
    parser.add_argument('--use_scst', type=bool, default=False, help='是否开启强化学习(SCST)')
    parser.add_argument('--scst_start_epoch', type=int, default=0, help='从第几个epoch开始进行SCST')

    # Data loader settings
    parser.add_argument('--dataset_name', type=str, default='iu_xray', choices=['iu_xray', 'mimic_cxr'],
                        help='the dataset to be used.')
    parser.add_argument('--max_seq_length', type=int, default=40, help='the maximum sequence length of the reports.')
...
def main():
    ...
    # 【新增逻辑】如果是 SCST 模式且指定了加载路径，在此处加载权重
    if args.load is not None:
        print(f"Loading pretrained model from {args.load} for SCST/Finetuning...")
