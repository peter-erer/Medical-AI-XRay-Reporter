...
class BaseCMN(AttModel):
    ...
    # --- 新增：强化学习核心采样函数 ---
# 修改 modules/base_cmn.py 中的 _sample 函数
    
    def _sample(self, fc_feats, att_feats, att_masks=None, opt={}, **kwargs):
        # 1. 参数合并逻辑
        if 'update_opts' in kwargs:
            opt.update(kwargs['update_opts'])
        opt.update(kwargs)
        ...
        # --- 核心防御逻辑：如果是 greedy (作为 RL 的 baseline)，强制设为 eval 模式 ---
        # 这样可以关闭所有的 Dropout，确保 Baseline 的分值是稳定的、无噪声的
        was_training = self.training
        if sample_method == 'greedy':
            self.eval()

        # 3. 准备特征
        fc_feats, att_feats, memory, att_masks = self._prepare_feature(fc_feats, att_feats, att_masks)
        ...
        # 4. 解码循环
        # 注意：这里我们使用 torch.no_grad() 的上下文取决于方法
        for t in range(max_seq_len):
            logprobs, state = self.core(it, fc_feats, att_feats, memory, state, att_masks)

            if sample_method == 'greedy':
                # 贪婪搜索不计算梯度，使用 detach 节省显存
                sampleLogProbs, it = torch.max(logprobs.detach(), 1)
                
            elif sample_method == 'sample':
                # 随机采样按分布采样
                probs = torch.exp(logprobs.detach() / temperature)
                it = torch.multinomial(probs, 1)
                # 提取选中词的 log_prob 并保留梯度
                sampleLogProbs = logprobs.gather(1, it).view(-1)
                it = it.view(-1)
    ...
    def core(self, it, fc_feats_ph, att_feats_ph, memory, state, mask):
        ...
        # 修改点：直接在这里计算 Log Softmax，方便 _sample 使用
        logit = self.logit(out[:, -1])
        logprobs = F.log_softmax(logit, dim=-1)

        if not self.training:
            self._save_attns(start=len(state) == 0)
            
        return logprobs, [ys.unsqueeze(0)] + past